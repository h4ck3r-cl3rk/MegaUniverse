...
public class Media {	
	...
	private void SwapBuffer(String t) {
		this.TempFile = t;
	}
	
	public static final String serialize(Media media) {
		media.TempFile = media.type.getValue().name() + "##_##" + media.name.getValue() + "##_##" + media.url.getValue();
		media.SwapBuffer(media.TempFile);
		return media.TempFile;
	}
